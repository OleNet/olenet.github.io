...
通过 Cross Lingual 的预训练来提升 NMT 的效果已经被上述工作证明行之有效，那么作为 SC 组和 MT 进行协作多语种翻译的项目中，应该扮演的角色应该更多的偏向基于Cross Lingual的预训练任务上。

由于 MT 目标时多个簇的语言互译，那么应用方式可能是
...
1. 通过预训练的cross lingual emb 提升效果
2. 解决通过 cross lingual pretrain 解决 low resource的问题。
...
1. 冷启多语模型->交付模型任意层
2. 我们找到一种单语热启的多语模型训练 -> 交付模型任意层
   1. 映射可以按照语言一个en一个<lang>训练
   2. 映射可以按照en和一个<lang_cluster>训练
   3. 也可以<lang_all>训练
3. 索性我们提出来一种任务，mt 网络带着额外的任务进行训练，multi-task learning -> 交付概念

训练单个语料的cross lingual encoder肯定不靠谱，因为没有办法一个一个的移植过去

所以当前的一对一不靠谱。

XLM-R 才可以，100种语言一起训练这种。
